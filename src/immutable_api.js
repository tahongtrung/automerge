...
function mergeContexts(...values) {
  ...
  var candidateContextLocal
  for (let value of values) {
    if (isWriteObject(value)) {
      const context = value._context
      const contextLocal = context.getIn(['state', 'opSet', 'local'])
      // Haven't considered a context yet, start with this one.
      if (candidateContext == undefined) {
        candidateContext = context
        candidateContextLocal = contextLocal
      // Prev candidate context has no local updates, can use this one.
      } else if (candidateContextLocal.size === 0) {
        candidateContext = context
        candidateContextLocal = contextLocal
      // This candidate context has no local updates, can use prev one.
      } else if (contextLocal.size === 0) {
        // No-op
      // Prev candidate context is a subset of this one, use this one.
      } else if (candidateContextLocal.isSubset(contextLocal)) {
        candidateContext = context
        candidateContextLocal = contextLocal
      // This candidate context is a subset of the prev one, use prev one.
      } else if (contextLocal.isSubset(candidateContextLocal)) {
        // No-op
      // The history requires merging which I don't know hot to do.
      } else {
        throw new Error('Do not know how to handle braided updates')
      }
    } else if (isReadObject(value)) {
      throw new Error('Do not know how to handle these objects')
    }
  }
  if (candidateContext === undefined) {
    throw new Error('Did not find a context to use')
  }
...
class WriteList {
  ...
  toString() {
    return `WriteList@${this._objectId}`
  }
...
class WriteMap {
  ...
  toString() {
    return `WriteMap@${this._objectId}`
  }
